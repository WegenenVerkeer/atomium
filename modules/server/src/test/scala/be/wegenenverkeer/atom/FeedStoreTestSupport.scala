...
trait FeedStoreTestSupport extends Matchers {
  ...
  def testFeedStorePaging(feedStore: AbstractFeedStore[String], pageSize: Int = 2): Unit = {
    ...
    lastPage.entries.map( _.content.value ) shouldEqual (pageSize to minId+1 by -1).map(i => i.toString).toList
    ...
    //check reverse chronological order of list contents
    feedPage.entries.map( _.content.value ) shouldEqual (2*pageSize to pageSize+minId+1 by -1).map(i => i.toString).toList
