 ...
 */
class AsyncFeedService[E, C <: Context](entriesPerPage: Int, feedStore: AsyncFeedStore[E, C]) {
   ...
   */
  def push(elements: Iterable[E])(implicit context: C): Future[Unit] = {
   ...
   * @param context the context, which is required for feed stores
   */
  def push(element: E)(implicit context: C): Future[Unit] = {
   ...
   * @param context the context, which is required for feed stores
   */
  def push(uuid: String, element: E)(implicit context: C): Future[Unit] = {
