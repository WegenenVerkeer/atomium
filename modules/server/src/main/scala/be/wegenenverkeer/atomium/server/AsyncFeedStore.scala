 ...
 */
trait AsyncFeedStore[E, C <: Context] {
   ...
   */
  def push(entries: Iterable[E])(implicit context: C) : Future[Unit]

  /**
   * push a single entry to the feed
   * @param entry the entry to push to the feed
   */
  def push(entry: E)(implicit context: C): Future[Unit] = {
    push(List(entry))
  }

  def push(uuid: String, entry: E)(implicit context: C): Future[Unit]
