...
object PlayJsonFormats {
  ...
  implicit val dateTimeFormat =
    Format[OffsetDateTime](
      ...
      Writes.temporalWrites[OffsetDateTime, DateTimeFormatter](TimestampFormat.WRITE_FORMAT))
  ...
  implicit val linkReads: Reads[Link] =
    ((__ \ "rel").read[String] and
      (__ \ "href").read[String])((rel, href) => new Link(rel, href))

  implicit val linkWrites: Writes[Link] =
    ((__ \ "rel").write[String] and
      (__ \ "href").write[String])(in => (in.getRel, in.getHref))

  implicit val generatorReads: Reads[Generator] =
    ((__ \ "text").read[String] and (__ \ "uri").read[String] and (__ \ "version").read[String])((text, uri, version) => new Generator(text, uri, version))

  implicit val generatorWrites: Writes[Generator] =
    ((__ \ "text").write[String] and
      (__ \ "uri").write[String] and
      (__ \ "version").write[String])((in) => (in.getText, in.getUri, in.getVersion))
  ...
  implicit val controlWrites: Writes[Control] = new Writes[Control] {
    override def writes(o: Control): JsValue =
      if (o.getDraft == null) {
  ...
  implicit def contentWrites[T: Writes]: Writes[Content[T]] =
    ((__ \ "value").write[T] and
      (__ \ "type").write[String])(in => (in.getValue, in.getType))

  implicit def contentReads[T: Reads]: Reads[Content[T]] =
    ((__ \ "value").read[T] and
      (__ \ "type").read[String])((value, `type`) => new Content[T](value, `type`))
  ...
  implicit def atomEntryWrites[T: Writes]: Writes[AtomEntry[T]] =
    ((__ \ "id").write[String] and
      ...
      (__ \ "_type").write[String])(in => (in.getId, in.getUpdated, in.getContent, in.getLinks.asScala.toList, "atom"))
  ...
  implicit def atomPubEntryWrites[T: Writes]: Writes[AtomPubEntry[T]] =
    ((__ \ "id").write[String] and
      ...
      (__ \ "_type").write[String])(in => (in.getId, in.getUpdated, in.getContent, in.getLinks.asScala.toList, in.getEdited, in.getControl, "atom-pub"))
  ...
  implicit def atomEntryReads[T: Reads]: Reads[AtomEntry[T]] =
    ((__ \ "id").read[String] and
      ...
      (__ \ "links").read[List[Link]])((id, updated, content, links) => new AtomEntry[T](id, updated, content, links.asJava))
  ...
  implicit def atomPubEntryReads[T: Reads]: Reads[AtomPubEntry[T]] =
    ((__ \ "id").read[String] and
      ...
      (__ \ "control").read[Control])((id, updated, content, links, edited, control) => new AtomPubEntry[T](id, updated, content, links.asJava, edited, control))
  ...
  implicit def feedWrites[T: Writes]: Writes[FeedPage[T]] =
    ((__ \ "id").write[String] and
      ...
      (__ \ "updated").write[OffsetDateTime] and
      (__ \ "links").write[List[Link]] and
      (__ \ "entries").write[List[Entry[T]]])(in =>
        (in.getId, new Url(in.getBase), Option(in.getTitle), Option(in.getGenerator), in.getUpdated, in.getLinks.asScala.toList, in.getEntries.asScala.toList))
  ...
  implicit def feedReads[T: Reads]: Reads[FeedPage[T]] =
    ((__ \ "id").read[String] and
      ...
      (__ \ "updated").read[OffsetDateTime] and
      (__ \ "links").read[List[Link]] and
      (__ \ "entries").read[List[Entry[T]]])((id, base, title, generator, updated, links, entries) =>
        new FeedPage[T](id, base.getPath, title.getOrElse(""), generator.getOrElse(null), updated, links.asJava, entries.asJava))
